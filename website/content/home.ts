import { CONTACT_URL } from "@/content/site";

export const homeContent = {
  hero: {
    title: "Salud Visual Accesible para Todos",
    subtitle:
      "Porque la discapacidad visual y la ceguera son un problema de salud pública crítico en México, trabajamos para ayudar a las comunidades vulnerables a acceder a atención oftalmológica de calidad.",
    ctaLabel: "Haz una Donación",
    ctaHref: CONTACT_URL,
    secondaryCtaLabel: "Conoce Más",
    secondaryCtaHref: "/about",
    videoUrl: "https://cdn.prod.website-files.com/68d7862ff86a6de94b452b46%2F68d7899c6f5fc3c0d8058a21_Hero%20Video-transcode.mp4",
    videoPoster: "/images/content/examen-nino.png"
  },
  intro: {
    heading: "Más de 11 millones de mexicanos viven con discapacidad visual",
    body: "El 80% de la pérdida de visión es prevenible o tratable. En Fundación Valdez Balli, trabajamos para reducir la ceguera evitable mediante un modelo sostenible y accesible de atención oftalmológica."
  },
  stats: [
    {
      icon: "https://www.rtsinternational.com/sites/default/files/2024-10/public.svg",
      value: "11.01 Millones",
      label: "De casos prevalentes de ceguera y discapacidad visual en México."
    },
    {
      icon: "https://www.rtsinternational.com/sites/default/files/2024-10/paid.svg",
      value: "80%",
      label: "De la pérdida de visión es prevenible o tratable con acceso adecuado."
    },
    {
      icon: "https://www.rtsinternational.com/sites/default/files/2024-10/headset_mic.svg",
      value: "26.73%",
      label: "De los casos de ceguera son causados por cataratas, tratables con cirugía."
    }
  ],
  financeSection: {
    eyebrow: "Nuestro Modelo de Atención",
    heading: "Centros de Atención Ocular accesibles y sostenibles",
    summary:
      "Implementamos un modelo inspirado en el exitoso Aravind Eye Care System de India, adaptado a las necesidades de México.",
    items: [
      {
        label: "Unidades de Atención Visual",
        title: "Consultorios permanentes en comunidades",
        description: "Equipados con tecnología básica para atención primaria oftalmológica, operados por profesionales de salud visual con apoyo de telemedicina.",
        mediaType: "image",
        mediaUrl: "/images/content/consultorio.png",
        metric: "Atención de lunes a sábado con horarios extendidos"
      },
      {
        label: "Caravanas de la Visión",
        title: "Unidades médicas móviles",
        description: "Llevamos la atención oftalmológica a comunidades de difícil acceso mediante caravanas equipadas para detección y referencia.",
        mediaType: "image",
        mediaUrl: "/images/content/caravana-rural.png",
        metric: "Cobertura en zonas rurales y marginadas"
      },
      {
        label: "Telemedicina",
        title: "Consultas remotas con especialistas",
        description: "Tecnología de diagnóstico remoto para facilitar la detección temprana y el seguimiento de pacientes en áreas remotas.",
        mediaType: "image",
        mediaUrl: "/images/content/caravana-urbana.png",
        metric: "Conexión con oftalmólogos especializados"
      },
      {
        label: "Programa de Cirugías",
        title: "Expediciones médico-quirúrgicas",
        description: "Programas vinculados con hospitales base para garantizar procedimientos quirúrgicos y tratamientos especializados.",
        mediaType: "image",
        mediaUrl: "/images/content/consultorio.png",
        metric: "Cirugías de cataratas y otros procedimientos"
      },
      {
        label: "Formación Profesional",
        title: "Capacitación de personal de salud",
        description: "Programas de formación para oftalmólogos, enfermeras especializadas, optometristas y técnicos en salud visual.",
        mediaType: "image",
        mediaUrl: "/images/content/equipo-microscopio.png",
        metric: "Profesionalización del sector y empleo local"
      },
      {
        label: "Modelo Sostenible",
        title: "Subsidios cruzados para acceso universal",
        description: "Los pacientes de mayores ingresos financian la atención de quienes no pueden costearla, garantizando acceso para todos.",
        mediaType: "image",
        mediaUrl: "/images/content/caravana-urbana.png",
        metric: "Sostenibilidad financiera a largo plazo"
      }
    ]
  },
  whyChoose: {
    heading: "¿A quién beneficiamos?",
    body: "Personas de comunidades vulnerables, especialmente mayores de 50 años, que no tienen acceso a servicios oftalmológicos de calidad debido a limitaciones económicas o geográficas.",
    ctaLabel: "Conocer más",
    ctaHref: "/services"
  },
  featureCards: [
    {
      title: "Detección Temprana",
      body: "Identificamos cataratas, glaucoma, retinopatía diabética y otros padecimientos en etapas iniciales.",
      imageUrl: "/images/content/ojo-catarata.png"
    },
    {
      title: "Prescripción de Lentes",
      body: "Exámenes de la vista y dispensación de lentes a precios accesibles para quienes lo necesitan.",
      imageUrl: "/images/content/lentes.png"
    },
    {
      title: "Caravanas de Salud",
      body: "Llevamos atención oftalmológica directamente a las comunidades más alejadas y vulnerables.",
      imageUrl: "/images/content/cirugia.png"
    },
    {
      title: "Cirugías de Cataratas",
      body: "Programas quirúrgicos que restauran la visión y mejoran la calidad de vida de los pacientes.",
      imageUrl: "/images/content/ojo-sano.png"
    },
    {
      title: "Educación Comunitaria",
      body: "Campañas de sensibilización sobre prevención de enfermedades visuales y buenas prácticas de cuidado ocular.",
      imageUrl: "/images/content/caravana-rural.png"
    }
  ],
  dashboard: {
    image: "/images/content/educacion.png",
    imageAlt: "Fundación Valdez Balli",
    heading: "Nuestro proceso de atención en 4 pasos",
    body: "Un modelo claro para brindar atención oftalmológica accesible:",
    steps: [
      "Detección y examen de la vista en nuestras unidades o caravanas.",
      "Diagnóstico y prescripción de tratamiento o lentes necesarios.",
      "Referencia a hospitales base para casos que requieren cirugía.",
      "Seguimiento continuo y monitoreo de pacientes con enfermedades crónicas."
    ],
    ctaLabel: "Contáctanos",
    ctaHref: "/contact"
  },
  testimonial: {
    heading: "Restaurando la visión, transformando vidas",
    quote:
      "Gracias a la Fundación Valdez Balli, pude recuperar mi visión y volver a trabajar. La cirugía de cataratas cambió mi vida por completo.",
    author: "Beneficiario de Fundación Valdez Balli"
  },
  contactMap: {
    heading: "Información de contacto",
    body: "Atención: contacto@fundacionvaldezballi.org | Monterrey, Nuevo León, México.",
    embedUrl:
      "https://www.google.com/maps?q=Monterrey,+Nuevo+León,+México&output=embed"
  }
};

export const globalTeamMembers = [
  {
    name: "Dr. Valdez",
    role: "Fundador y Director Médico"
  },
  {
    name: "Equipo Médico",
    role: "Oftalmólogos Especialistas"
  },
  {
    name: "Personal de Salud",
    role: "Optometristas y Técnicos"
  },
  {
    name: "Coordinadores",
    role: "Gestión de Vision Centers"
  },
  {
    name: "Trabajadores Comunitarios",
    role: "Vinculación y Educación"
  },
  {
    name: "Voluntarios",
    role: "Apoyo en Caravanas"
  }
];
